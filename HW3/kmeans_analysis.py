...
def kmeans_until_max_iter(X, y, num_clusters, distance_fn, max_iterations):
    np.random.seed(69)
    X_len = len(X)
    random_idx_list = np.random.choice(range(X_len), size=num_clusters, replace=False)
    centroids = X[random_idx_list]
...
if __name__ == "__main__":
    ...
    print('Cosine Distance:')
    cos_ite_centroids, cos_ite_pts_in_centroids, cos_ite_sse = kmeans_until_max_iter(X, y, num_clusters=num_clusters, distance_fn=cosine_distance, max_iterations=100)
    print()

    print('Generalized Jaccard Distance:')
    jac_ite_centroids, jac_ite_pts_in_centroids, jac_ite_sse = kmeans_until_max_iter(X, y, num_clusters=num_clusters, distance_fn=generalized_jaccard_distance, max_iterations=100)
    print()
    ...
    print(f'SSE - Gen Jaccard Distance - Until max iterations: {jac_ite_sse}')
    print()